...
static void extract_lnor_loop_bmesh(const MeshRenderData *mr,
                                    int l,
                                    BMLoop *UNUSED(loop),
                                    void *data)
{
...
static void extract_lnor_loop_mesh(const MeshRenderData *mr,
                                   int l,
...
const MeshExtract extract_uv_tan = {extract_uv_tan_init,
                                    ...
                                    NULL,
                                    NULL,
                                    MR_DATA_POLY_NOR | MR_DATA_LOOP_NOR | MR_DATA_LOOPTRI,
                                    false};
...
static void extract_mesh_analysis_finish(const MeshRenderData *mr, void *buf, void *UNUSED(data))
{
  ...
  /* TODO: This is only doing proper statvis for the cage. A good enhancement would be to
   * compute the statvis here and on the final mesh. This could potentially avoid a copy and
  ...
  uchar(*l_weight)[4] = (uchar(*)[4])vbo->data;
  ...
  BM_ITER_MESH (efa, &f_iter, mr->bm, BM_FACES_OF_MESH) {
    BM_ITER_ELEM (loop, &l_iter, efa, BM_LOOPS_OF_FACE) {
      uchar *weight = (is_vertex_data) ?
                          mr->edit_bmesh->derivedVertColor[BM_elem_index_get(loop->v)] :
                          mr->edit_bmesh->derivedFaceColor[BM_elem_index_get(loop->f)];
      copy_v4_v4_uchar(l_weight[BM_elem_index_get(loop)], weight);
    }
  }
