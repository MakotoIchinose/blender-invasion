  ...
  for (lip = ba->LinkedLines.first; lip; lip = lip->next) {
    ...
    if (rv != nrl->L && rv != nrl->R) {
      if (nrl->Flags & LANPR_EDGE_FLAG_INTERSECTION) {
        if (rv->FrameBufferCoord[0] == nrl->L->FrameBufferCoord[0] &&
            rv->FrameBufferCoord[1] == nrl->L->FrameBufferCoord[1]) {
          *new_rv = LANPR_OTHER_RV(nrl, nrl->L);
          return nrl;
        }
        elif (rv->FrameBufferCoord[0] == nrl->R->FrameBufferCoord[0] &&
              rv->FrameBufferCoord[1] == nrl->R->FrameBufferCoord[1])
        {
                                              ...
                                              float dist_threshold)
{
  ...
  for (rlci = from->Item.next; rlci != to; rlci = next_rlci) {
    ...
    if (next_rlci && (next_rlci->OcclusionLevel != rlci->OcclusionLevel ||
                      next_rlci->LineType != rlci->LineType))
      continue;
...
void lanpr_NO_THREAD_chain_feature_lines(LANPR_RenderBuffer *rb, float dist_threshold)
{
  ...
  for (rl = rb->all_render_lines.first; rl; rl = rl->Item.next) {
    ...
    while (ba && (new_rl = lanpr_get_connected_render_line(ba, new_rv, &new_rv))) {
      ...
      if (new_rv == new_rl->L) {
        for (rls = new_rl->Segments.last; rls; rls = rls->Item.prev) {
          float px, py;
          px = tnsLinearItp(
              new_rl->L->FrameBufferCoord[0], new_rl->R->FrameBufferCoord[0], rls->at);
          py = tnsLinearItp(
              new_rl->L->FrameBufferCoord[1], new_rl->R->FrameBufferCoord[1], rls->at);
          lanpr_push_render_line_chain_point(
              rb, rlc, px, py, N, new_rl->Flags, rls->OcclusionLevel);
        }
      }
      elif (new_rv == new_rl->R)
      {
        ...
        lanpr_push_render_line_chain_point(rb,
                                           rlc,
                                           new_rl->R->FrameBufferCoord[0],
                                           new_rl->R->FrameBufferCoord[1],
                                           N,
                                           new_rl->Flags,
                                           last_occlusion);
      }
      ba = lanpr_get_point_bounding_area(
          rb, new_rv->FrameBufferCoord[0], new_rv->FrameBufferCoord[1]);
    }
    ...
    lanpr_append_render_line_chain_point(
        rb, rlc, rl->R->FrameBufferCoord[0], rl->R->FrameBufferCoord[1], N, rl->Flags, 0);
      ...
      if (new_rv == new_rl->L) {
        ...
        for (rls = new_rl->Segments.last; rls; rls = rls->Item.prev) {
          ...
          last_occlusion = rls->Item.prev ?
                               ((LANPR_RenderLineSegment *)rls->Item.prev)->OcclusionLevel :
                               0;
