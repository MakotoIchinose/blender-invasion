...
static void eevee_lightcache_create_resources(EEVEE_LightBake *lbake, EEVEE_LightCache *lcache)
{
	...
	lbake->bounce_count = BKE_collection_engine_property_value_get_int(props, "gi_diffuse_bounces");
	lbake->vis_res      = BKE_collection_engine_property_value_get_int(props, "gi_visibility_resolution");
	lbake->rt_res       = BKE_collection_engine_property_value_get_int(props, "gi_cubemap_resolution");
...
void EEVEE_lightcache_bake_job(void *custom_data, short *UNUSED(stop), short *UNUSED(do_update), float *UNUSED(progress))
{
	...
	eevee_lightcache_create_resources(lbake, lcache);
