...
/* Revisions. */
static FileDirEntryView *rna_AssetRevision_views_add(
    FileDirEntryRevision *revision /*, ReportList *reports,*/)
{
...
/* Variants. */
static FileDirEntryRevision *rna_AssetVariant_revisions_add(
    FileDirEntryVariant *variant /*, ReportList *reports,*/)
{
...
static FileDirEntryVariant *rna_AssetEntry_variants_add(
    FileDirEntry *entry /*, ReportList *reports,*/)
{
...
static void rna_AssetList_entries_remove(FileDirEntryArr *dirlist,
                                         ReportList *reports,
                                         PointerRNA *ptr)
{
  FileDirEntry *entry = ptr->data;
  ...
  if (!BLI_remlink_safe(&dirlist->entries, entry)) {
    BKE_report(
        reports, RPT_ERROR, "Trying to remove an entry from a list which does not contain it!");
    return;
  }
...
static bool rna_ae_load_pre(AssetEngine *engine,
                            AssetUUIDList *uuids,
...
static bool rna_ae_sort_filter(AssetEngine *engine,
                               const bool use_sort,
                               const bool use_filter,
                               FileSelectParams *params,
                               FileDirEntryArr *entries_r)
{
...
static bool rna_ae_entries_block_get(AssetEngine *engine,
...
static bool rna_ae_entries_uuid_get(AssetEngine *engine,
                                    AssetUUIDList *uuids,
                                    FileDirEntryArr *entries_r)
{
                                           ...
                                           StructFreeFunc free)
{
  ...
  if (strlen(identifier) >= sizeof(dummyaet.idname)) {
    BKE_reportf(reports,
                RPT_ERROR,
                "Registering asset engine class: '%s' is too long, maximum length is %d",
                identifier,
                (int)sizeof(dummyaet.idname));
    return NULL;
  }
...
static void rna_def_asset_uuid_list(BlenderRNA *brna)
{
  ...
  prop = RNA_def_int(srna,
                     ...
                     "Asset engine version those uuids were generated from",
                     0,
                     INT_MAX);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
...
static void rna_def_asset_view(BlenderRNA *brna)
{
  ...
  RNA_def_property_string_funcs(
      prop, "rna_AssetView_name_get", "rna_AssetView_name_length", "rna_AssetView_name_set");
  RNA_def_property_ui_text(prop, "Name", "");

  prop = RNA_def_property(srna, "description", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetView_description_get",
                                "rna_AssetView_description_length",
  ...
  prop = RNA_def_int(srna,
                     ...
                     INT_MAX,
                     "Size",
                     "Size (in bytes, special value '-1' means 'no size')",
                     -1,
                     INT_MAX);
  ...
  prop = RNA_def_int(srna,
                     ...
                     INT_MAX,
                     "Timestamp",
                     "In seconds since the epoch (should be copy of revision timestamp typically)",
                     0,
                     INT_MAX);
  RNA_def_property_int_funcs(
      prop, "rna_AssetView_timestamp_get", "rna_AssetView_timestamp_set", NULL);
}
...
static void rna_def_asset_views(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  RNA_def_property_pointer_funcs(
      prop, "rna_AssetRevision_active_view_get", "rna_AssetRevision_active_view_set", NULL, NULL);
  ...
  RNA_def_property_ui_text(
      prop, "Active Index", "Index of asset's view curently active (selected)");
  ...
  parm = RNA_def_pointer(
      func, "view", "AssetView", "New View", "New asset entry variant revision view");
...
static void rna_def_asset_revision(BlenderRNA *brna)
{
  ...
  RNA_def_property_string_funcs(prop,
                                "rna_AssetRevision_comment_get",
                                "rna_AssetRevision_comment_length",
                                "rna_AssetRevision_comment_set");
  RNA_def_property_ui_text(prop, "Comment", "");

  prop = RNA_def_int(
      srna, "timestamp", 0, 0, INT_MAX, "Timestamp", "In seconds since the epoch", 0, INT_MAX);
  RNA_def_property_int_funcs(
      prop, "rna_AssetRevision_timestamp_get", "rna_AssetRevision_timestamp_set", NULL);
...
static void rna_def_asset_revisions(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  RNA_def_property_pointer_funcs(prop,
                                 "rna_AssetVariant_active_revision_get",
                                 "rna_AssetVariant_active_revision_set",
                                 NULL,
                                 NULL);
  ...
  RNA_def_property_ui_text(
      prop, "Active Index", "Index of asset's revision curently active (selected)");
  ...
  /* return arg */
  parm = RNA_def_pointer(
      func, "revision", "AssetRevision", "New Revision", "New asset entry variant revision");
  RNA_def_function_return(func, parm);
}
...
static void rna_def_asset_variant(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "name", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetVariant_name_get",
                                "rna_AssetVariant_name_length",
                                "rna_AssetVariant_name_set");
  RNA_def_property_ui_text(prop, "Name", "");

  prop = RNA_def_property(srna, "description", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetVariant_description_get",
                                "rna_AssetVariant_description_length",
...
static void rna_def_asset_variants(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_pointer_funcs(
      prop, "rna_AssetEntry_active_variant_get", "rna_AssetEntry_active_variant_set", NULL, NULL);
  ...
  RNA_def_property_ui_text(
      prop, "Active Index", "Index of asset's variant curently active (selected)");
  ...
  /* return arg */
  parm = RNA_def_pointer(
      func, "variant", "AssetVariant", "New Variant", "New asset entry variant");
  RNA_def_function_return(func, parm);
}
...
static void rna_def_asset_entry(BlenderRNA *brna)
{
  ...
  static EnumPropertyItem asset_revision_types[] = {
      ...
      {FILE_TYPE_BLENDERLIB,
       ...
       0,
       "Blender Library",
       "An entry that is part of a .blend file"},
      {0, NULL, 0, NULL, NULL}};
  ...
  RNA_def_struct_sdna(srna, "FileDirEntry");
  RNA_def_struct_ui_text(
      srna, "Asset Entry", "A single asset item (quite similar to a file path)");
  //  RNA_def_struct_ui_icon(srna, ICON_NONE);  /* XXX TODO */
  ...
  prop = RNA_def_property(srna, "name", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(
      prop, "rna_AssetEntry_name_get", "rna_AssetEntry_name_length", "rna_AssetEntry_name_set");
  RNA_def_property_ui_text(prop, "Name", "");

  prop = RNA_def_property(srna, "description", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetEntry_description_get",
                                "rna_AssetEntry_description_length",
                                "rna_AssetEntry_description_set");
  RNA_def_property_ui_text(prop, "Description", "");

  prop = RNA_def_property(srna, "relpath", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetEntry_relpath_get",
                                "rna_AssetEntry_relpath_length",
...
static void rna_def_asset_entries(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_pointer_funcs(
      prop, "rna_AssetList_active_entry_get", "rna_AssetList_active_entry_set", NULL, NULL);
...
static void rna_def_asset_list(BlenderRNA *brna)
{
  ...
  prop = RNA_def_int(srna,
                     ...
                     INT_MAX,
                     "Entries Number",
                     "Total number of available entries/assets, *not the length of 'entries'!*",
                     0,
                     INT_MAX);
  ...
  prop = RNA_def_int(srna,
                     ...
                     0,
                     INT_MAX,
                     "Filtered Entries Number",
                     "Total number of visible entries/assets, *not the length of 'entries'!*",
                     0,
                     INT_MAX);
  ...
  RNA_def_property_ui_text(
      prop, "Root Path", "Root directory from which all asset entries come from");
}
...
static void rna_def_asset_engine(BlenderRNA *brna)
{
  ...
  RNA_def_struct_register_funcs(
      srna, "rna_AssetEngine_register", "rna_AssetEngine_unregister", "rna_AssetEngine_instance");
  ...
  RNA_def_property_boolean_funcs(
      prop, "rna_AssetEngine_is_dirty_sorting_get", "rna_AssetEngine_is_dirty_sorting_set");
  RNA_def_property_ui_text(
      prop, "Dirty Sorting", "FileBrowser shall call AE's sorting function on next draw");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_FILE_PARAMS, NULL);

  prop = RNA_def_property(srna, "is_dirty_filtering", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(
      prop, "rna_AssetEngine_is_dirty_filtering_get", "rna_AssetEngine_is_dirty_filtering_set");
  RNA_def_property_ui_text(
      prop, "Dirty Filtering", "FileBrowser shall call AE's filtering function on next draw");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_FILE_PARAMS, NULL);
  ...
  RNA_def_int(func,
              ...
              INT_MAX,
              "",
              "Job ID (JOB_ID_UNSET to get engine status itself)",
              AE_JOB_ID_INVALID,
              INT_MAX);
  parm = RNA_def_enum(func,
                      "status_return",
                      asset_engine_status_types,
                      0,
                      "",
                      "Status of given job or whole engine");
  ...
  RNA_def_function_ui_description(
      func, "Get progress of a given job, or all running ones (between 0.0 and 1.0)");
  RNA_def_function_flag(func, FUNC_REGISTER);
  RNA_def_int(func,
              "job_id",
              AE_JOB_ID_UNSET,
              ...
              INT_MAX,
              "",
              "Job ID (JOB_ID_UNSET to get average progress of all running jobs)",
              AE_JOB_ID_INVALID,
              INT_MAX);
  ...
  RNA_def_int(func,
              ...
              INT_MAX,
              "",
              "Job ID (JOB_ID_UNSET to kill all)",
              AE_JOB_ID_INVALID,
              INT_MAX);
  ...
  RNA_def_int(func,
              ...
              INT_MAX,
              "",
              "Job ID (JOB_ID_UNSET to start a new one)",
              AE_JOB_ID_INVALID,
              INT_MAX);
  RNA_def_pointer(func,
                  ...
                  "AssetList",
                  "",
                  "List of asset entries proposed to user by the asset engine");
  parm = RNA_def_int(func,
                     ...
                     INT_MAX,
                     "",
                     "Job ID (if JOB_ID_INVALID, job is assumed already finished)",
                     AE_JOB_ID_INVALID,
                     INT_MAX);
  RNA_def_function_output(func, parm);
  ...
  RNA_def_function_ui_description(
      func, "Check for already loaded asset status (is updated, still valid, etc.)");
  RNA_def_function_flag(func, FUNC_REGISTER | FUNC_ALLOW_WRITE);
  RNA_def_int(func,
              "job_id",
              AE_JOB_ID_UNSET,
  ...
  RNA_def_function_ui_description(func,
                                  "Ensure given UUIDs are really available "
  ...
  RNA_def_function_ui_description(
      func, "Pre-process given assets identifiers to make them loadable by Blender");
  RNA_def_function_flag(func, FUNC_REGISTER_OPTIONAL | FUNC_ALLOW_WRITE);
  RNA_def_pointer(func, "uuids", "AssetUUIDList", "", "Identifiers of assets to 'make real'");
  RNA_def_pointer(
      func, "entries", "AssetList", "", "List of actual, existing paths that Blender can load");
  ...
  RNA_def_pointer(
      func, "uuids", "AssetUUIDList", "", "Identifiers and data-blocks of loaded assets");
  parm = RNA_def_boolean(func, "success_return", false, "", "Success");
  RNA_def_function_output(func, parm);
  ...
  RNA_def_function_ui_description(
      func, "Check if given path is valid (as in, can be listed) for this engine");
  RNA_def_function_flag(func, FUNC_REGISTER_OPTIONAL | FUNC_ALLOW_WRITE);
  RNA_def_pointer(func,
                  "entries",
                  "AssetList",
                  "",
                  "Fake List of asset entries (only use/modify its root_path!)");
  ...
  parm = RNA_def_boolean(
      func, "changed_return", false, "", "Whether list of available entries was changed");
  RNA_def_function_output(func, parm);

  /* Block of entries by-index getter callback */
  func = RNA_def_function(srna, "entries_block_get", NULL);
  RNA_def_function_ui_description(
      func, "Get a block of entries/assets by its (sorted/filtered) start/end index");
  RNA_def_function_flag(func, FUNC_REGISTER_OPTIONAL | FUNC_ALLOW_WRITE);
  RNA_def_int(func,
              "start_index",
              0,
              ...
              INT_MAX,
              "",
              "Index of first entry (asset) to get (included)",
              0,
              INT_MAX);
  RNA_def_int(func,
              "end_index",
              ...
              INT_MAX,
              "",
              "Index of last entry (asset) to get (excluded)",
              0,
              INT_MAX);
  RNA_def_pointer(func,
                  "entries",
