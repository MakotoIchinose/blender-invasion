...
static ID *is_dupid(ListBase *lb, ID *id, const char *name)
{
  ...
  for (idtest = lb->first; idtest; idtest = idtest->next) {
    ...
    /* Virtual lib IDs are considered as local ones here, since we bulk-add them to virtual library
     * datablocks, we need to ensure ourselves there is no name collision there. */
