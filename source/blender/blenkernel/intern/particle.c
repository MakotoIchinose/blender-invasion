...
void psys_check_group_weights(ParticleSettings *part)
{
	...
	if (part->ren_as == PART_DRAW_GR && part->dup_group && part->dup_group->scene_layer->object_bases.first) {
		...
		while (dw) {
			if (dw->ob == NULL || !BKE_group_object_exists(part->dup_group, dw->ob)) {
				Base *base = BLI_findlink(&part->dup_group->scene_layer->object_bases, dw->index);
				if (base != NULL) {
					dw->ob = base->object;
				}
				else {
