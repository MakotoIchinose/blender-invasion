...
struct LatticeDeformData *psys_create_lattice_deform_data(ParticleSimulationData *sim)
{
	...
	if (psys_in_edit_mode(sim->psys, sim->eval_ctx->active_object) == 0) {
		Object *lattice = NULL;
...
bool psys_in_edit_mode(ParticleSystem *psys, Object *active_object)
{
	return (active_object &&
	        (active_object->mode & OB_MODE_PARTICLE_EDIT) &&
	        psys == psys_get_current(active_object) &&
        ...
        float cfra, const bool editupdate, const bool use_render_params)
{
	...
	if (psys_in_edit_mode(psys, sim->eval_ctx->active_object)) {
...
static void psys_thread_create_path(ParticleTask *task, struct ChildParticle *cpa, ParticleCacheKey *child_keys, int i)
{
	...
	ParticleCacheKey **pcache = psys_in_edit_mode(psys, ctx->sim.eval_ctx->active_object) && psys->edit ? psys->edit->pathcache : psys->pathcache;
...
void psys_get_particle_on_path(ParticleSimulationData *sim, int p, ParticleKey *state, const bool vel)
{
	...
	if (p < totpart) {
		...
		/* otherwise interpolate with other means */
			...
			pind.dm = psys_in_edit_mode(psys, sim->eval_ctx->active_object) ? NULL : psys->hair_out_dm;
