...
void BKE_group_copy_data(Main *UNUSED(bmain), Group *group_dst, const Group *group_src, const int flag)
{
	...
	SceneCollection *master_collection_src = BKE_collection_group_master(group_src);
	SceneCollection *master_collection_dst = BKE_collection_group_master(group_dst);

	/* Recursively creates a new SceneCollection tree. */
	BKE_collection_copy_data(master_collection_dst, master_collection_src,
	                         flag_subdata);

	BKE_scene_layer_copy_data(group_dst->scene_layer, group_src->scene_layer,
...
bool BKE_group_object_exists(Group *group, Object *ob)
{
	if (group == NULL || ob == NULL) {
		...
		return (BLI_findptr(&group->scene_layer->object_bases, ob, offsetof(Base, object)));
	}
}
...
void BKE_group_tag_recalc(Group *UNUSED(group))
{
	/* Do nothing for now. */
}
