        ...
        EffectorWeights *weights, bool for_simulation)
{
	...
	if (weights->group) {
		scene_layer = weights->group->scene_layer;
	}
	else if (eval_ctx) {
		scene_layer = eval_ctx->scene_layer;
	}
	else {
		/* eval_ctx is NULL during deg build */
		scene_layer = BKE_scene_layer_context_active_PLACEHOLDER(scene);
	}

	for (base = FIRSTBASE_NEW(scene_layer); base; base = base->next) {
