...
void *get_bone_from_selectbuffer(
        Scene *scene, Object *active_object, const unsigned int *buffer, short hits,
	...
	for (i = 0; i < hits; i++) {
		...
		if (!(hitresult & BONESEL_NOSEL)) {
			if (hitresult & BONESEL_ANY) {  /* to avoid including objects in selection */
				...
				if (obedit == NULL || active_object != obedit) {
					/* no singular posemode, so check for correct object */
					if (active_object->select_color == (hitresult & 0xFFFF)) {
						bone = get_indexed_bone(active_object, hitresult);
...
void *get_nearest_bone(bContext *C, const int xy[2], bool findunsel)
{
	...
	if (hits > 0)
		return get_bone_from_selectbuffer(vc.scene, vc.obact, buffer, hits, findunsel, true);
