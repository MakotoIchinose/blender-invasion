...
/* mode is 's' start, or 'e' end, or 'u' use */
/* if end, ob can be NULL */
int ED_real_mesh_mirror_spatial_table(Object *ob, BMEditMesh *em, Mesh *mesh, const float co[3], char mode)
{
	if (mode == 'u') {        /* use table */
		if (MirrKdStore.tree == NULL)
		...
		if (MirrKdStore.tree) /* happens when entering this call without ending it */
			ED_real_mesh_mirror_spatial_table(ob, em, mesh, co, 'e');
...
bool ED_real_mesh_mirrtopo_recalc_check(Mesh *me, Mesh *dm, MirrTopoStore_t *mesh_topo_store)
{
	const bool is_editmode = (me->edit_btmesh != NULL);
	int totvert;
	int totedge;
        ...
        const bool skip_em_vert_array_init)
{
	...
	ED_real_mesh_mirrtopo_init(me, fake_mesh, mesh_topo_store, skip_em_vert_array_init);
