                                           ...
                                           const TaskParallelTLS *__restrict UNUSED(tls))
{
  ...
  bool use_orco = ELEM(data->brush->sculpt_tool,
                       SCULPT_TOOL_GRAB,
                       SCULPT_TOOL_ROTATE,
                       SCULPT_TOOL_THUMB,
                       SCULPT_TOOL_POSE);
...
static void sculpt_update_brush_delta(UnifiedPaintSettings *ups, Object *ob, Brush *brush)
{
  ...
  if (ELEM(tool,
    ...
    if (ELEM(tool, SCULPT_TOOL_GRAB, SCULPT_TOOL_THUMB, SCULPT_TOOL_POSE)) {
...
static bool sculpt_any_smooth_mode(const Brush *brush, StrokeCache *cache, int stroke_mode)
{
  ...
  return ((stroke_mode == BRUSH_STROKE_SMOOTH) || (cache && cache->alt_smooth) ||
          ...
          (brush->sculpt_tool == SCULPT_TOOL_RELAX) || (brush->sculpt_tool == SCULPT_TOOL_POSE));
}
...
bool sculpt_stroke_get_geometry_info(bContext *C, StrokeGeometryInfo *out, const float mouse[2])
{
  ...
  if (srd.hit) {
    ...
    ss->active_vertex_index = srd.output.active_vertex_mesh_index;
...
int sculpt_mesh_filter_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  BKE_sculpt_update_object_for_edit(depsgraph, ob, needs_pmap, true);
  ...
  BKE_pbvh_vertex_iter_begin(ss->pbvh, node, vd, PBVH_ITER_UNIQUE)
  {
    ...
    switch (mode) {
      ...
      case MASK_FILTER_GROW:
        ...
        sculpt_vertex_neighbors_iter_end(ni) *vd.mask = max;
        CLAMP(*vd.mask, 0.0f, 1.0f);
        break;
      case MASK_FILTER_SHRINK:
        ...
        sculpt_vertex_neighbors_iter_end(ni) *vd.mask = min;
        CLAMP(*vd.mask, 0.0f, 1.0f);
        break;
...
static char sculpt_get_vertex_symm_area(SculptSession *ss, float co[3], float pivot[3])
{
  ...
  // add custom symmetry origin later...
  copy_v3_v3(vco, co);
  if (vco[0] < 0) {
                                     ...
                                     const TaskParallelTLS *__restrict UNUSED(tls))
{
  ...
  BKE_pbvh_vertex_iter_begin(ss->pbvh, node, vd, PBVH_ITER_ALL)
  {
    ...
    sub_v3_v3v3(disp, ss->pivot_pos, ss->init_pivot_pos);
    mul_v3_fl(disp, fade);
    sculpt_flip_transform_by_symm_area(disp, NULL, symm, symm_area, ss->init_pivot_pos);
    add_v3_v3(final_disp, disp);

    /* rotate */
    unit_m4(transform);
    unit_m4(transform_aux);
    sub_qt_qtqt(rot, ss->pivot_rot, ss->init_pivot_rot);
    copy_v3_v3(final_pivot_pos, ss->pivot_pos);
    sculpt_flip_transform_by_symm_area(final_pivot_pos, rot, symm, symm_area, ss->init_pivot_pos);
    quat_to_mat4(transform, rot);
    translate_m4(transform_aux, final_pivot_pos[0], final_pivot_pos[1], final_pivot_pos[2]);
    invert_m4_m4(transform_aux_inv, transform_aux);
    copy_v3_v3(val, orig_co);
    mul_m4_v3(transform_aux_inv, val);
    mul_m4_v3(transform, val);
    mul_m4_v3(transform_aux, val);
    sub_v3_v3v3(disp, val, orig_co);
    mul_v3_fl(disp, fade);
