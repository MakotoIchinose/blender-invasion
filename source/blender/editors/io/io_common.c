...
ExportSettings * io_common_construct_default_export_settings(struct bContext *C,
	...
	settings->flatten_hierarchy   = RNA_boolean_get(op->ptr, "flatten");
	...
	settings->export_vweigths     = RNA_boolean_get(op->ptr, "export_vweigths");
...
int io_common_export_exec(struct bContext *C, struct wmOperator *op,
	...
	return exporter(C, settings) ? OPERATOR_FINISHED : OPERATOR_CANCELLED;
}
